...
def data_preprocess_TFIDF(X):
    ## TODO: need test
    ### TFIDF: for DNN - keras
    vectorizer_x = pickle.load(open("../tokenizer/TFIDFVectorizer.pkl", "rb"))
...
def do_test_sklearn_knn(query_size):
    model_path = "../trained_models/20News_KNN.pkl"
...
def do_test_sklearn_svm(query_size):
    model_path = "../trained_models/20News_SVM_probs.pkl"
    model = load_model(model_path)
    test_X, _ = prepare_query(seed=0, size=query_size)
    pred_y = model.predict_proba(test_X)
    # pred_y = regularize_output(pred_y,CATEGORY_NUM)
...
def do_test_sklearn_bagging(query_size):
    model_path = "../trained_models/20News_bagging.pkl"
    model = load_model(model_path)
    test_X, _ = prepare_query(seed=0, size=query_size)
    pred_y = model.predict_proba(test_X)
    # pred_y = regularize_output(pred_y,CATEGORY_NUM)
...
def do_test_sklearn_boost(query_size):
    model_path = "../trained_models/20News_boost.pkl"
    model = load_model(model_path)
    test_X, _ = prepare_query(seed=0, size=query_size)
    pred_y = model.predict_proba(test_X)
    # pred_y = regularize_output(pred_y,CATEGORY_NUM)
...
def do_test_sklearn_random_forest(query_size):
    model_path = "../trained_models/20News_randomForest.pkl"
    model = load_model(model_path)
    test_X, _ = prepare_query(seed=0, size=query_size)
    pred_y = model.predict_proba(test_X)
    # pred_y = regularize_output(pred_y,CATEGORY_NUM)
...
def do_test_sklearn_decision_tree(query_size):
    model_path = "../trained_models/20News_DecisionTree.pkl"
    model = load_model(model_path)
    test_X, _ = prepare_query(seed=0, size=query_size)
    pred_y = model.predict_proba(test_X)
    # pred_y = regularize_output(pred_y,CATEGORY_NUM)
...
def do_test_keras_DNN(query_size):
    model_path = "../trained_models/20News_DNN.keras"
    model = load_model(model_path)
    test_X, _ = prepare_query(seed=0, size=query_size)
...
def do_test_keras_CNN(query_size):
    model_path = "../trained_models/20News_CNN.keras"
    model = load_model(model_path)
    test_X, _ = prepare_query(seed=0, size=query_size)
...
def do_test_keras_RNN(query_size):
    model_path = "../trained_models/20News_RNN.keras"
    model = load_model(model_path)
    test_X, _ = prepare_query(seed=0, size=query_size)
    test_X = data_preprocess_Tokenizer(test_X)
    pred_y = model.predict(test_X)
...
def do_test_keras_RCNN(query_size):
    model_path = "../trained_models/20News_RCNN.keras"
    model = load_model(model_path)
    test_X, _ = prepare_query(seed=0, size=query_size)
    test_X = data_preprocess_Tokenizer(test_X)
    pred_y = model.predict(test_X)
...
def collect_outputs_of_type_label(q_size):
    save_path = "./outputs/query_%d/type_label" % q_size
    if not os.path.exists(save_path):
        os.makedirs(save_path)

    preds = do_test_sklearn_svm(query_size=q_size)
    preds = convert_output_type(preds, OUTPUT_TYPE.TYPE_LABEL)
    save_results_to_txt(preds, "%s/svm_train.csv" % save_path)

    preds = do_test_sklearn_knn(query_size=q_size)
    preds = convert_output_type(preds, OUTPUT_TYPE.TYPE_LABEL)
    save_results_to_txt(preds, "%s/knn_train.csv" % save_path)

    preds = do_test_sklearn_bagging(query_size=q_size)
    preds = convert_output_type(preds, OUTPUT_TYPE.TYPE_LABEL)
    save_results_to_txt(preds, "%s/bagging_train.csv" % save_path)

    preds = do_test_sklearn_boost(query_size=q_size)
    preds = convert_output_type(preds, OUTPUT_TYPE.TYPE_LABEL)
    save_results_to_txt(preds, "%s/boost_train.csv" % save_path)

    preds = do_test_sklearn_random_forest(query_size=q_size)
    preds = convert_output_type(preds, OUTPUT_TYPE.TYPE_LABEL)
    save_results_to_txt(preds, "%s/srandom_forest_train.csv" % save_path)

    preds = do_test_sklearn_decision_tree(query_size=q_size)
    preds = convert_output_type(preds, OUTPUT_TYPE.TYPE_LABEL)
    save_results_to_txt(preds, "%s/decision_tree_train.csv" % save_path)

    preds = do_test_keras_DNN(query_size=q_size)
    preds = convert_output_type(preds, OUTPUT_TYPE.TYPE_LABEL)
    save_results_to_txt(preds, "%s/DNN_train.csv" % save_path)

    preds = do_test_keras_CNN(query_size=q_size)
    preds = convert_output_type(preds, OUTPUT_TYPE.TYPE_LABEL)
    save_results_to_txt(preds, "%s/CNN_train.csv" % save_path)

    preds = do_test_keras_RNN(query_size=q_size)
    preds = convert_output_type(preds, OUTPUT_TYPE.TYPE_LABEL)
    preds = randomize_output(preds)
    save_results_to_txt(preds, "%s/RNN_train.csv" % save_path)

    preds = do_test_keras_RCNN(query_size=q_size)
    preds = convert_output_type(preds, OUTPUT_TYPE.TYPE_LABEL)
    save_results_to_txt(preds, "%s/RCNN_train.csv" % save_path)


def collect_outputs_of_type_rank(q_size):
    save_path = "./outputs/query_%d/type_rank" % q_size
    if not os.path.exists(save_path):
        os.makedirs(save_path)
...
def collect_outputs_of_type_probs(q_size):
    save_path = "./outputs/query_%d/type_probs" % q_size
    if not os.path.exists(save_path):
        os.makedirs(save_path)
...
def collect_in_ones(q_size):
    save_path_label = "./outputs/query_%d/type_label" % q_size
    save_path_rank = "./outputs/query_%d/type_rank" % q_size
    save_path_probs = "./outputs/query_%d/type_probs" % q_size
    ...
    preds = do_test_sklearn_svm(query_size=q_size)
    preds = convert_output_type(preds, OUTPUT_TYPE.TYPE_PROBS)
    save_results_to_txt(preds, "%s/svm_train.csv" % save_path_probs)

    preds = do_test_sklearn_knn(query_size=q_size)
    preds = convert_output_type(preds, OUTPUT_TYPE.TYPE_PROBS)
    save_results_to_txt(preds, "%s/knn_train.csv" % save_path_probs)

    preds = do_test_sklearn_bagging(query_size=q_size)
    preds = convert_output_type(preds, OUTPUT_TYPE.TYPE_PROBS)
    save_results_to_txt(preds, "%s/bagging_train.csv" % save_path_probs)

    preds = do_test_sklearn_boost(query_size=q_size)
    preds = convert_output_type(preds, OUTPUT_TYPE.TYPE_PROBS)
    save_results_to_txt(preds, "%s/boost_train.csv" % save_path_probs)

    preds = do_test_sklearn_random_forest(query_size=q_size)
    preds = convert_output_type(preds, OUTPUT_TYPE.TYPE_PROBS)
    save_results_to_txt(preds, "%s/srandom_forest_train.csv" % save_path_probs)

    preds = do_test_sklearn_decision_tree(query_size=q_size)
    preds = convert_output_type(preds, OUTPUT_TYPE.TYPE_LABEL)
    save_results_to_txt(preds, "%s/decision_tree_train.csv" % save_path_probs)

    # ======= DL models ======

    preds = do_test_keras_DNN(query_size=q_size)
    preds_probs = convert_output_type(preds, OUTPUT_TYPE.TYPE_PROBS)
    save_results_to_txt(preds_probs, "%s/DNN_train.csv" % save_path_probs)

    preds = do_test_keras_CNN(query_size=q_size)
    preds_probs = convert_output_type(preds, OUTPUT_TYPE.TYPE_PROBS)
    save_results_to_txt(preds_probs, "%s/CNN_train.csv" % save_path_probs)

    preds = do_test_keras_RNN(query_size=q_size)
    preds_probs = convert_output_type(preds, OUTPUT_TYPE.TYPE_PROBS)
    save_results_to_txt(preds_probs, "%s/RNN_train.csv" % save_path_probs)

    preds = do_test_keras_RCNN(query_size=q_size)
    preds_probs = convert_output_type(preds, OUTPUT_TYPE.TYPE_PROBS)
    save_results_to_txt(preds_probs, "%s/RCNN_train.csv" % save_path_probs)
